...
class PreResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, low_dim=128, hashbits=32):
    ...
    def forward(self, x, lin=0, lout=5, do_recon=False, has_out=True):
        ...
        if lout > 4:
            ...
            if do_recon:
                ...
                feat_lowD = self.l2norm(feat_lowD)
                if has_out:
                    return logits, output_h
                else:
                    return feat_lowD


def PreResNet18SH(num_class=10, low_dim=20,hashbits=32):
    return PreResNet(PreActBlock, [2, 2, 2, 2],
                     num_classes=num_class,
                     low_dim=low_dim, hashbits=hashbits)


def PreResNet34SH(num_class=10, low_dim=20,hashbits=32):
