...
class WBCELoss(torch.nn.Module):
    def __init__(self,lam):
        ...
        self.lamd_weight = lam
    
    def forward(self,outputs,targets):
        losses = F.binary_cross_entropy(outputs, targets, reduction="none")
        with torch.no_grad():
            weight = em_weight(losses,self.lamd_weight)
        return torch.mean(weight* torch.mean(losses,dim=1),dim=0)



class GCELossH(torch.nn.Module):
    def __init__(self, hashbits, q=0.7, gpu=None):
        super(GCELoss, self).__init__()
        self.device = torch.device('cuda:%s'%gpu) if gpu else torch.device('cpu')
...
def em_weight(outputs,lmbda):
    std =  torch.std(outputs,dim=1)
    return torch.exp(-lmbda * std)
